...
<main class="min-h-screen flex items-center min-w-xs">
  <div class="sm:max-w-3xl mx-auto p-6 text-white bg-[#2b2d31] rounded-md shadow-lg">
    ...
    <div class="flex items-center gap-4 bg-[#313338] p-4 rounded-md mb-6">
      ...
      />
      <div class="w-full">
        <p class="text-lg font-medium">Discord name: {userSignedIn.name}</p>
        <p class="text-lg font-medium">Discord username: {userSignedIn.username}</p>
        <p class="text-lg font-medium">Discord ID: {userSignedIn.id?.toString()}</p>
        <p class="text-lg font-medium">Discord email: {userSignedIn.email}</p>
    ...
    <div class="bg-[#313338] p-6 rounded-md mb-6">
      ...
      <div class="my-6">
        ...
        <div class="flex items-center bg-[#1e1f22] rounded-md">
          ...
          <input type="text" value={userSignedIn.username ?? 'username'} placeholder="Username" class="bg-transparent text-white ml-2 flex-1 ring-green-500 ring-1 rounded focus:ring-0 p-1" id="username_text" />
        </div>
      </div>
